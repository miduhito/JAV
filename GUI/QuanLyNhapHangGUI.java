...
public class QuanLyNhapHangGUI extends RoundedPanel {
    ...
    public void loadButtonActionListener(){
        ...
        addButton.addActionListener(_ -> {
            ...
            if (!soLuong.split(" ")[0].matches("\\d+(\\.\\d+)?")) {
                JOptionPane.showMessageDialog(null, "Số lượng phải là số hợp lệ!", "Lỗi", JOptionPane.WARNING_MESSAGE);
                return;
            }

            Double soLuongMoi = Double.parseDouble(soLuong.split(" ")[0]);
            boolean daTonTai = false;

            for (int i = 0; i < cartTableModel.getRowCount(); i++) {
                if (cartTableModel.getValueAt(i, 0).equals(maNguyenLieu)) {
                    Double soLuongHienTai = Double.parseDouble(cartTableModel.getValueAt(i, 2).toString().split(" ")[0]);
            ...
            if (response == JOptionPane.YES_OPTION) {
                try {
                    ...
                    for (int i = 0; i < cartTable.getRowCount(); i++) {
                        ...
                        Double soLuongNhap = Double.parseDouble(cartTable.getValueAt(i, 2).toString().split(" ")[0]); // Số lượng
    ...
    private void showDetailForm(String maPhieuNhap){
        ...
        NhanVienDTO nhanVienNhap = nhanVienBUS.getNhanVienById(phieuNhap.getMaNhanVien());
        ...
        JTextField nhanVienNhapField = new JTextField(nhanVienNhap.getTenNhanVien() + " (Mã nhân viên:  " + nhanVienNhap.getMaNhanVien() + ")" );
        ...
        for (ChiTietPhieuNhapDTO chiTietPhieuNhap: danhSachChiTietPhieuNhap){
            NguyenLieuDTO nguyenLieu = nguyenLieuBUS.getNguyenLieuById(chiTietPhieuNhap.getMaNguyenLieu());
            Vector<String> rowData = new Vector<>();
    ...
    private void showAdvancedSearchDialog() {
        ...
        ArrayList<NhanVienDTO> danhSachNhanVien = new ArrayList<>(nhanVienBUS.getAllNhanVien());
        Vector<String> nhanVienData = new Vector<>();
        for (NhanVienDTO nhanVien: danhSachNhanVien){
            nhanVienData.add(nhanVien.getMaNhanVien() + " - " + nhanVien.getTenNhanVien());
        }
        ...
        searchButton.addActionListener(e -> {
            ...
            for (PhieuNhapDTO phieuNhap : filteredList) {
                ...
                phieuNhapTableModel.addRow(new Object[]{
                    ...
                    maNV + " - " + nhanVienBUS.getNhanVienById(maNV).getTenNhanVien(),
