        ...
        try {
            ...
            if (mode.equals("CaLam")) {
                ...
                for (NhanVienDTO nv : nvBUS.getAllNhanVien()) {
                    danhSachNhanVien.add(nv.getMaNhanVien() + " - " + nv.getTenNhanVien());
                }
...
// ======================= FORM SỬA CA LÀM ========================
    private void ShowFormSua(JButton editButton, JTable table, String mode) {
        ...
        if (mode.equals("CaLam")) {
            ...
            // Boolean trangThai = (Boolean) table.getValueAt(selectedRow, 4);

            CaLamDTO caLam = clBUS.getDataById(maCa);
            String moTa = caLam.getMoTa();
            String gioBD = caLam.getGioBD();
            String gioKT = caLam.getGioKT();
            Boolean trangThai = caLam.getTrangThai();

            System.out.println(gioBD);
            System.out.println(gioKT);
            ...
            field2Label.setText("Giờ bắt đầu:");
            field2Component = new JComboBox<>(new String[]{"08:00", "09:00", "10:00", "11:00", "12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00"});
            ((JComboBox<?>) field2Component).setSelectedItem(gioBD.substring(0, 5));

            field3Label.setText("Giờ kết thúc:");
            field3Box.setModel(new DefaultComboBoxModel<>(new String[]{"12:00", "13:00", "14:00", "15:00", "16:00", "17:00", "18:00", "19:00", "20:00", "21:00", "22:00", "23:00"}));
            field3Box.setSelectedItem(gioKT.substring(0, 5));
        }
        else {
        ...
        if (mode.equals("CaLam")) {
            ...
            saveButton.addActionListener(_ -> {
                ...
                newCaLam.setGioBD(gioBatDauBox.getSelectedItem() + "");
                newCaLam.setGioKT(field3Box.getSelectedItem() + "");
                newCaLam.setTrangThai("Hiệu lực".equals(trangThaiBox.getSelectedItem()));

                if (clBUS.update(newCaLam)) {
                    clBUS.loadDataTable(tableModelCL);
                    formSua.dispose();
                }
                editButton.setEnabled(true);
