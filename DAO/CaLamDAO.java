...
public class CaLamDAO implements DAO_Interface<CaLamDTO> {
    ...
    public ArrayList<CaLamDTO> getData(){
        ...
        try {
            ...
            if(connDB.openConnectDB()){
                String query = "SELECT * FROM calam ORDER BY gioBD ASC, gioKT ASC ";
    ...
    public String generateID() {
        String newMaCa = "CA001"; // Giá trị mặc định nếu bảng trống
        try {
            if (connDB.openConnectDB()) {
                String query = "SELECT maCa FROM calam ORDER BY maCa DESC LIMIT 1"; // Lấy mã lớn nhất
                Statement stmt = connDB.conn.createStatement();
                ResultSet rs = stmt.executeQuery(query);
                ...
                if (rs.next()) {
                    ...
                    // Tách phần số từ mã
                    String numberPart = lastMaCa.substring(2); // Bỏ "CA", lấy phần số
                    ...
                    newMaCa = String.format("CA%03d", number);
                }
